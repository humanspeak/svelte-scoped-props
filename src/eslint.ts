...
function buildCharacterMap(original: string, generated: string): Array<number | null> {
  ...
  mapBlock(original, generated, originalCursor, original.length, generatedCursor, generated.length, map);
...
function findNextAnchor(
  ...
  for (const length of [24, 16, 12, 8, 4]) {
    for (let generatedIndex = generatedStart; generatedIndex <= generated.length - length; generatedIndex += 1) {
...
function longestCommonSubsequencePairs(original: string, generated: string): Array<[number, number]> {
...
function matchEqualLines(
  ...
  for (let originalIndex = originalLines.length - 1; originalIndex >= 0; originalIndex -= 1) {
    for (let generatedIndex = generatedLines.length - 1; generatedIndex >= 0; generatedIndex -= 1) {
      const cursor = originalIndex * width + generatedIndex;
