...
import scopedPropsEslint, {
  configs,
  createScopedPropsProcessor,
  processor
} from '../src/eslint.js';
...
<style>.parent-owned{color:purple}</style>`;
    const processed = readBlock(localProcessor.preprocess(source, filename));
    const generatedLoc = locFromIndex(processed, processed.indexOf('__svelte_scoped_props_marker'));
      ...
      [
        [
          ...
          },
          {
            ...
            column: generatedLoc.column,
            fix: {
              range: [generatedHelperStart, generatedClassValueIndex + 'classValue'.length],
              text: 'classValue'
            }
          }
        ]
