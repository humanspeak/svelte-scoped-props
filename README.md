...
- It preserves scoped CSS selectors by injecting an uncalled snippet marker. That marker does not render, but it does leave a small unused function in the compiled module. The marker is synthesized from the component's own selectors, so compound (`.page-rows.paged`), combinator (`.a .b`, `.a > .b`, `span.a + button.b`), and element-qualified (`button.foo`) scoped selectors survive Svelte's unused-CSS pruning; attribute selectors and `:has()` are still not matched.
